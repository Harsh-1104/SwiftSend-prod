...
async function findData(apikey, column) {
  try {
    return await new Promise((resolve, reject) => {
      conn.query(
        `SELECT ${column} FROM users WHERE apikey = '${apikey}'`,
        (error, result) => {
          if (error || result.length <= 0)
            return reject(status.internalservererror().status_code);
  ...
  new GoogleStrategy(
    {
      ...
      callbackURL: `${
        process.argv[2] ? DomainName : `http://localhost:8081`
    ...
    conn.query(
      ...
      function (err, result) {
        ...
        if (result.length > 0) {
          ...
          conn.query(
            `INSERT INTO users (apikey,uname,email,password,phone,phoneverify,country,state,city,registrationDate,image) VALUES(?,?,?,?,?,?,?,?,?,?,?)`,
            [
              apikey,
              Ac_name,
              Ac_mail,
    ...
    if (isValidapikey) {
      ...
      for (var i in clientarr) {
        ...
        if (i != clientarr.length - 1) {
          query += `('${id}','${apikey}','${clientarr[i].name}','${
            clientarr[i].email
          }','${clientarr[i].phone ? clientarr[i].phone : 0}','${
            req.body.iid
          }'),`;
        } else {
          query += `('${id}','${apikey}','${clientarr[i].name}','${
    ...
    tableData(
      ...
      (result) => {
        if (result.status_code == 404) {
          conn.query(
            `INSERT INTO instance(instance_id,i_name,apikey,token,create_date,isActive) values('${id}','${name}','${apikey}','${token}',CURRENT_DATE,0)`,
            ...
            tableData(
              ...
              (result) => {
                ...
                remaining_days = Math.ceil(
                  Math.round(latest - current_date) / (1000 * 60 * 60 * 24)
                );
...
app.post("/AgentReplyToTicket", async (req, res) => {
  ...
  conn.query(`select * from company`, async function (err, result) {
    if (err || result.length <= 0)
      return res.send(status.internalservererror());
    if (result.length > 0) {
      const sender = {
